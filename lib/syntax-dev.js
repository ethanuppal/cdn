...
const SyntaxHighlighter = {
    ...
    htmlSyntaxHighlight: function(code) {
        ...
        html = html.replaceAll(/(async|defer)(?=\W)/g, '<span class="attribute">$&</span>');
        code.innerHTML = html;
    },
//
    ...
    jsSyntaxHighlight: function(code) {
        ...
        html = html.replaceAll(/(var|let|if|for|while|do|return|const|async|(class )|function|continue|of)(?=\W)/g, '<span class="keyword">$&</span>');
        html = html.replaceAll(/(?<=[^a-zA-Z_]|^)(-|\+)?[0-9]+(\.[0-9]+)?/g, `<span class='number'>$&</span>`);
        html = html.replaceAll(/\w+(?=\()/g, '<span class="function">$&</span>');
